#!/usr/bin/env python3
"""
China Tourism Academy Spider
中国旅游研究院数据爬虫

Target: http://www.ctaweb.org.cn/
Data: Tourism statistics, visitor numbers, revenue data, market analysis
Focus: Domestic tourism, inbound/outbound tourism, holiday travel data

Architecture:
  - Primary: HTML scraping with Scrapling Fetcher
  - Anti-bot: Browser impersonation, rate limiting
  - Output: SQLite DB + JSON export
  - Special: Extracts tourism revenue and visitor volume data
"""
...
logger = logging.getLogger("tourism")

BASE_DIR = Path(__file__).resolve().parent
DATA_DIR = BASE_DIR / "data"
OUTPUT_DIR = BASE_DIR / "output"
DB_PATH = DATA_DIR / "tourism.db"
JSON_STATS_PATH = OUTPUT_DIR / "tourism_statistics.json"
JSON_NEWS_PATH = OUTPUT_DIR / "tourism_news.json"

TARGET_URLS = {
    "statistics": [
        "http://www.ctaweb.org.cn/data/tourism/",
        "http://www.ctaweb.org.cn/data/domestic/",
        "http://www.ctaweb.org.cn/data/inbound/",
    ],
    "news": [
        "http://www.ctaweb.org.cn/news/industry/",
        "http://www.ctaweb.org.cn/news/policy/",
    ],
    "reports": [
        "http://www.ctaweb.org.cn/report/analysis/",
    ],
}
...
def init_db():
    ...
    conn.execute("""
        CREATE TABLE IF NOT EXISTS tourism_statistics (
            ...
            UNIQUE(period, tourism_type, indicator_name, region, source_url)
        )
    """)
    conn.execute("""
...
def identify_tourism_type(text: str) -> dict | None:
    for cn_name, en_name in TOURISM_TYPES.items():
        if cn_name in text:
            return {"tourism_type": en_name, "tourism_type_cn": cn_name}
    return None
...
def extract_statistics(html: str, url: str) -> list[dict]:
    ...
    for table in sel.css("table"):
        ...
        if not any(kw in header_text for kw in ["游客", "收入", "人次", "亿元", "旅游", "增长"]):
            continue
        ...
        for row in rows[1:]:
            ...
            type_info = identify_tourism_type(cell_text)
            if not type_info:
                type_info = {"tourism_type": "general", "tourism_type_cn": "综合"}
            ...
            if numbers:
                try:
                    ...
                    if "亿人次" in cell_text:
                        item["unit"] = "亿人次"
                    elif "万人次" in cell_text:
                        item["unit"] = "万人次"
                    elif "亿元" in cell_text:
                        item["unit"] = "亿元"
                    elif "万亿" in cell_text:
            ...
            if "游客" in header_text or "人次" in cell_text:
                item["indicator_name"] = "visitor_count"
            elif "收入" in header_text or "消费" in cell_text:
                item["indicator_name"] = "tourism_revenue"
...
def get_tourism_data(
    ...
    """Fetch data from China Tourism Academy.

    Args:
        include_stats: Whether to fetch tourism statistics.
    ...
    logger.info("Starting Tourism spider")
...
if __name__ == "__main__":
    ...
    if results["statistics"]:
        print(f"\nTourism Statistics ({len(results['statistics'])} records):")
        for stat in results["statistics"][:10]:
            print(f"  {stat.get('period', 'N/A'):>12s} | {stat.get('tourism_type_cn', 'N/A'):>8s} | "
