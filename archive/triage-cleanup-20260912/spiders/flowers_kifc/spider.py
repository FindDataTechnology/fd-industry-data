...
def run_flowers_kifc(limit: int | None = None) -> list[dict]:
    """Entry point for fd-open-data-protocol dispatch."""
    from scrapling import Fetcher

    items: list[dict] = []
    fetcher = Fetcher(auto_match=False, impersonate="chrome")
    spider = KIFCFlowerSpider()

    target_paths = [""] + PRICE_PAGE_PATHS
    for path in target_paths:
        url = f"http://www.kifc.cn{path}" if path else "http://www.kifc.cn"
        try:
            response = fetcher.get(url, timeout=30, stealthy_headers=True)
            if response.status != 200:
                continue
            extracted = spider._extract_from_tables(response)
            if extracted:
