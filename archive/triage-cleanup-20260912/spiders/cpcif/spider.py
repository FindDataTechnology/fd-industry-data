...
def run_cpcif(limit: int | None = None, include_reports: bool = True, include_news: bool = True) -> list[dict]:
    """Entry point for fd-open-data-protocol dispatch."""
    conn = init_db()
    items = []

    result = get_cpcif_data(include_reports=include_reports, include_news=include_news)

    # Collect all data into a single list
    all_records = []
    all_records.extend(result.get("production", []))
    all_records.extend(result.get("market", []))
    all_records.extend(result.get("trade", []))
    all_records.extend(result.get("reports", []))
    all_records.extend(result.get("news", []))
...
if __name__ == "__main__":
    results = run_cpcif(include_reports=True, include_news=True)
