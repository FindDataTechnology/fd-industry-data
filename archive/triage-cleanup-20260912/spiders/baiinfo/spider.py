...
def run_baiinfo(limit: int = 100) -> list[dict]:
    """Entry point for fd-open-data-protocol dispatch."""
    conn = init_db()
    result = get_baiinfo_data(include_market=True, include_news=True)

    all_items = []
    all_items.extend(result.get("prices", [])[:limit])

    remaining = limit - len(all_items)
    if remaining > 0 and result.get("market"):
        all_items.extend(result["market"][:remaining])

    remaining = limit - len(all_items)
    if remaining > 0 and result.get("news"):
        all_items.extend(result["news"][:remaining])
