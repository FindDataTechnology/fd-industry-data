...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "oschina.db")
OUTPUT_DIR = os.path.join(os.path.dirname(__file__), "output")

START_URLS = [
    "https://www.oschina.net/project/stats",
]
CUSTOM_UAS = [
...
class OschinaSpider(Spider):
    ...
    def _save_sqlite(self, item):
        ...
        try:
            ...
            create_sql = f"""CREATE TABLE IF NOT EXISTS oschina_data (
            ...
            insert_sql = f"INSERT OR IGNORE INTO oschina_data ({', '.join(cols)}) VALUES ({placeholders})"
    ...
    def _export(self):
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM oschina_data")
...
def run_oschina(urls=None, limit: int = 100) -> list[dict]:
    ...
    try:
        ...
        if os.path.exists(DB_PATH):
            ...
            conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM oschina_data LIMIT ?", (limit,))
            results = [dict(row) for row in cursor.fetchall()]
            conn.close()
    except Exception as e:
        spider_instance.logger.error(f"Error in run_oschina: {e}")
