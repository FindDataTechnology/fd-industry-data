...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "github-opendata-china.db")
...
NAME = "github-opendata-china"

class GithubopendatachinaSpider(Spider):
    name = "github-opendata-china"
    ...
    def _save_sqlite(self, item):
        ...
        try:
            ...
            create_sql = f"""CREATE TABLE IF NOT EXISTS github-opendata-china_data (
            ...
            insert_sql = f"INSERT OR IGNORE INTO github-opendata-china_data ({', '.join(cols)}) VALUES ({placeholders})"
    ...
    def _export(self):
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM github-opendata-china_data")
...
def run_github_opendata_china(urls=None, save=True):
    """Run the github-opendata-china spider. Returns True on success."""
    spider = GithubopendatachinaSpider()
...
if __name__ == "__main__":
    ...
    spider_instance = GithubopendatachinaSpider()
