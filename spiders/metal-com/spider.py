...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "metal-com.db")
...
class MetalcomSpider(Spider):
    ...
    def _save_sqlite(self, item):
        ...
        try:
            ...
            create_sql = f"""CREATE TABLE IF NOT EXISTS metal-com_data (
            ...
            insert_sql = f"INSERT OR IGNORE INTO metal-com_data ({', '.join(cols)}) VALUES ({placeholders})"
    ...
    def _export(self):
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM metal-com_data")
...
if __name__ == "__main__":
    ...
    spider_instance = MetalcomSpider()
