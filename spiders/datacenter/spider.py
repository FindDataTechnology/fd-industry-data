...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "datacenter.db")
...
class DatacenterSpider(Spider):
    ...
    def _save_sqlite(self, item):
        ...
        try:
            ...
            create_sql = f"""CREATE TABLE IF NOT EXISTS datacenter_data (
            ...
            insert_sql = f"INSERT OR IGNORE INTO datacenter_data ({', '.join(cols)}) VALUES ({placeholders})"
    ...
    def _export(self):
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM datacenter_data")
...
if __name__ == "__main__":
    ...
    spider_instance = DatacenterSpider()
