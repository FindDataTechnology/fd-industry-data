...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "hqcdc.db")
OUTPUT_DIR = os.path.join(os.path.dirname(__file__), "output")

START_URLS = [
    "https://www.hqcdc.org.cn/",
]
CUSTOM_UAS = [
...
class HqcdcSpider(Spider):
    ...
    def _save_sqlite(self, item):
        ...
        try:
            ...
            create_sql = f"""CREATE TABLE IF NOT EXISTS hqcdc_data (
            ...
            insert_sql = f"INSERT OR IGNORE INTO hqcdc_data ({', '.join(cols)}) VALUES ({placeholders})"
    ...
    def _export(self):
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM hqcdc_data")
