#!/usr/bin/env python3
"""Data Science China Datasets Scrapling Spider."""
...
START_URLS: List[str] = [
    "https://github.com/Data-Science-China/datasets",
]
...
def save_to_sqlite(records: List[Dict[str, Any]]) -> None:
    ...
    for key, value in sample.items():
        ...
        placeholders.append("?")
    table_name = "data_science_china_records"
...
def parse_response(response: Response) -> List[Dict[str, Any]]:
    ...
    try:
        readme_content = response.css("article.markdown-body")
        if readme_content:
            text = readme_content[0].text.strip() if hasattr(readme_content[0], "text") else str(readme_content[0])
            record = {
        ...
        repo_files = response.css("div.js-navigation-item")
        for f in repo_files:
            name_el = f.css("a.Link--primary")
            if name_el:
                name = name_el[0].text.strip() if hasattr(name_el[0], "text") else str(name_el[0])
                record = {
                    "source_url": response.request.url,
                    "file_name": name,
                    "scraped_at": datetime.now().isoformat(),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Scrapling spider for Data Science China Datasets")
