...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "fred-data.db")
...
class FreddataSpider(Spider):
    ...
    def _save_sqlite(self, item):
        ...
        try:
            ...
            create_sql = f"""CREATE TABLE IF NOT EXISTS fred_economic (
            ...
            insert_sql = f"INSERT OR IGNORE INTO fred_economic ({col_str}) VALUES ({placeholders})".format(
                col_str=", ".join(cols), 
    ...
    def _export(self):
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM fred_economic")
...
if __name__ == "__main__":
    ...
    spider_instance = FreddataSpider()
