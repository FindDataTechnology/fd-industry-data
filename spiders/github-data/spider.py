...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "github-data.db")
...
NAME = "GitHub Awesome China Data"

class GithubdataSpider(Spider):
    name = "github-data"
    ...
    def _save_sqlite(self, item):
        ...
        try:
            ...
            create_sql = f"""CREATE TABLE IF NOT EXISTS awesome_china_data (
            ...
            insert_sql = f"INSERT OR IGNORE INTO awesome_china_data ({col_str}) VALUES ({placeholders})".format(
                col_str=", ".join(cols), 
    ...
    def _export(self):
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM awesome_china_data")
...
if __name__ == "__main__":
    ...
    spider_instance = GithubdataSpider()
