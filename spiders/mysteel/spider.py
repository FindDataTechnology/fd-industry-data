...
START_URLS: List[str] = [
    ...
    "https://www.mysteel.com/price/",
    "https://www.mysteel.com/info/",
]
...
def parse_response(response: Response) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        price_items = response.css(".price-list .price-item, .market-data .item")
        
        for item in price_items:
            parsed_data = {
                "source_url": response.url,
                "page_title": title,
                "product_name": item.css(".product-name::text").get("").strip(),
                "price": item.css(".price::text").get("").strip(),
                "change": item.css(".change::text").get("").strip(),
                "date": item.css(".date::text").get("").strip(),
