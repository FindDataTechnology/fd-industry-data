#!/usr/bin/env python3
"""GitHub China Data Hub Scrapling Spider."""
...
START_URLS: List[str] = [
    "https://github.com/opendata/china-data-hub",
]
...
def save_to_sqlite(records: List[Dict[str, Any]]) -> None:
    ...
    for key, value in sample.items():
        ...
        placeholders.append("?")
    table_name = "github_china_data_hub_records"
...
def parse_response(response: Response) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if title_el:
            title = title_el[0].text.strip() if hasattr(title_el[0], "text") else str(title_el[0])

        repo_items = response.css("div.Box-row, article.Box-row, li.Box-row")
        for item in repo_items:
            name_el = item.css("a[href]")
            desc_el = item.css("p, div.f4")
            record = {}
            if name_el:
                link = name_el[0]
                record["name"] = link.text.strip() if hasattr(link, "text") else str(link)
                href = link.attrib.get("href", "") if hasattr(link, "attrib") else ""
                record["url"] = "https://github.com" + href if href and not href.startswith("http") else href
            if desc_el:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Scrapling spider for GitHub China Data Hub")
