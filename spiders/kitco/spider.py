...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "kitco.db")
...
class KitcoSpider(Spider):
    ...
    def _save_sqlite(self, item):
        ...
        try:
            ...
            create_sql = f"""CREATE TABLE IF NOT EXISTS kitco_data (
            ...
            insert_sql = f"INSERT OR IGNORE INTO kitco_data ({', '.join(cols)}) VALUES ({placeholders})"
    ...
    def _export(self):
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM kitco_data")
