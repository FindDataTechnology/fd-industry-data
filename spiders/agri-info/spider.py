...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "agri-info.db")
OUTPUT_DIR = os.path.join(os.path.dirname(__file__), "output")

START_URLS = [
    "http://www.agri-info.cn/",
]
CUSTOM_UAS = [
...
class AgriinfoSpider(Spider):
    ...
    def _save_sqlite(self, item):
        ...
        try:
            ...
            create_sql = f"""CREATE TABLE IF NOT EXISTS agri-info_data (
            ...
            insert_sql = f"INSERT OR IGNORE INTO agri-info_data ({', '.join(cols)}) VALUES ({placeholders})"
    ...
    def _export(self):
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM agri-info_data")
...
if __name__ == "__main__":
    ...
    spider_instance = AgriinfoSpider()
