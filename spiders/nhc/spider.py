...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "nhc.db")
...
class NhcSpider(Spider):
    ...
    def _save_sqlite(self, item):
        ...
        try:
            ...
            create_sql = f"""CREATE TABLE IF NOT EXISTS nhc_data (
            ...
            insert_sql = f"INSERT OR IGNORE INTO nhc_data ({', '.join(cols)}) VALUES ({placeholders})"
    ...
    def _export(self):
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM nhc_data")
