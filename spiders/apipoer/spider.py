...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "apipoer.db")
OUTPUT_DIR = os.path.join(os.path.dirname(__file__), "output")

START_URLS = [
    "https://open.apipoer.com/",
]
CUSTOM_UAS = [
...
class ApipoerSpider(Spider):
    ...
    def _save_sqlite(self, item):
        ...
        try:
            ...
            create_sql = f"""CREATE TABLE IF NOT EXISTS apipoer_data (
            ...
            insert_sql = f"INSERT OR IGNORE INTO apipoer_data ({', '.join(cols)}) VALUES ({placeholders})"
    ...
    def _export(self):
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM apipoer_data")
...
def run_apipoer(limit: int = 100) -> list[dict]:
    """Entry point for fd-open-data-protocol dispatch."""
    from fd_industry_data.runners import run_scrapling_spider
    return run_scrapling_spider(ApipoerSpider(), START_URLS, limit=limit)
