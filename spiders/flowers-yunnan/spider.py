#!/usr/bin/env python3
"""Yunnan Flowers (云南省花卉产业信息网) Scrapling Spider."""
...
START_URLS: List[str] = [
    "https://flowers.yunnan.gov.cn",
    "https://flowers.yunnan.gov.cn/price",
    "https://flowers.yunnan.gov.cn/market",
]
...
def save_to_sqlite(records: List[Dict[str, Any]]) -> None:
    ...
    if not records:
        conn.close()
        return
    sql_create = """CREATE TABLE IF NOT EXISTS flowers_yunnan (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        scraped_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        source_url TEXT, page_title TEXT, flower_name TEXT,
        price TEXT, unit TEXT, category TEXT,
        origin TEXT, publish_date TEXT
    )"""
    cursor.execute(sql_create)
    for record in records:
        cursor.execute(
            "INSERT INTO flowers_yunnan (source_url, page_title, flower_name, price, unit, category, origin, publish_date) VALUES (?,?,?,?,?,?,?,?)",
            (record.get("source_url"), record.get("page_title"), record.get("flower_name"),
             record.get("price"), record.get("unit"), record.get("category"),
             record.get("origin"), record.get("publish_date"))
        )
    conn.commit()
    conn.close()
...
def parse_response(response: Response) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        price_items = response.css(".price-list .item, .flower-item, table tr, .market-data .row")
        for item in price_items:
            cells = item.css("td::text").getall()
            if len(cells) >= 3:
                parsed_data = {
                    "source_url": response.url,
                    "page_title": title,
                    "flower_name": cells[0].strip() if cells else "",
                    "price": cells[1].strip() if len(cells) > 1 else "",
                    "unit": cells[2].strip() if len(cells) > 2 else "",
                    "category": cells[3].strip() if len(cells) > 3 else "",
                    "origin": cells[4].strip() if len(cells) > 4 else "",
                    "publish_date": cells[5].strip() if len(cells) > 5 else "",
                    "scraped_at": datetime.now().isoformat(),
                }
                results.append(parsed_data)
            else:
                name = item.css(".flower-name::text, .name::text, h3::text").get("")
                price = item.css(".price::text, .value::text").get("")
                if name or price:
                    parsed_data = {
                        "source_url": response.url,
                        "page_title": title,
                        "flower_name": name.strip(),
                        "price": price.strip(),
                        "unit": item.css(".unit::text").get("").strip(),
                        "category": item.css(".category::text").get("").strip(),
                        "origin": item.css(".origin::text").get("").strip(),
                        "publish_date": item.css(".date::text, time::text").get("").strip(),
                        "scraped_at": datetime.now().isoformat(),
                    }
                    results.append(parsed_data)
        if not results:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Scrapling spider for Yunnan Flowers (云南省花卉产业信息网)")
