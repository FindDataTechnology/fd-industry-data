#!/usr/bin/env python3
"""NBS Stats Data Query (国家统计局数据查询) Scrapling Spider."""
...
def save_to_sqlite(records: List[Dict[str, Any]]) -> None:
    ...
    if not records:
        conn.close()
        return
    sql_create = """CREATE TABLE IF NOT EXISTS nbs_stats_data (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        scraped_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        source_url TEXT, indicator_name TEXT, indicator_value TEXT,
        period TEXT, unit TEXT, region TEXT,
        category TEXT
    )"""
    cursor.execute(sql_create)
    for record in records:
        cursor.execute(
            "INSERT INTO nbs_stats_data (source_url, indicator_name, indicator_value, period, unit, region, category) VALUES (?,?,?,?,?,?,?)",
            (record.get("source_url"), record.get("indicator_name"), record.get("indicator_value"),
             record.get("period"), record.get("unit"), record.get("region"), record.get("category"))
        )
    conn.commit()
    conn.close()
...
def parse_response(response: Response) -> List[Dict[str, Any]]:
    ...
    try:
        table_rows = response.css("table tr, .data-table tr, #tablelist tr")
        for row in table_rows:
            cells = row.css("td::text").getall()
            if len(cells) >= 2:
                parsed_data = {
                    "source_url": response.url,
                    "indicator_name": cells[0].strip() if cells else "",
                    "indicator_value": cells[1].strip() if len(cells) > 1 else "",
                    "period": cells[2].strip() if len(cells) > 2 else "",
                    "unit": cells[3].strip() if len(cells) > 3 else "",
                    "region": cells[4].strip() if len(cells) > 4 else "",
                    "category": "",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Scrapling spider for NBS Stats Data Query (国家统计局数据查询)")
