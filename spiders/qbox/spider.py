...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "qbox.db")
...
class QboxSpider(Spider):
    ...
    def _save_sqlite(self, item):
        ...
        try:
            ...
            create_sql = f"""CREATE TABLE IF NOT EXISTS qbox_data (
            ...
            insert_sql = f"INSERT OR IGNORE INTO qbox_data ({', '.join(cols)}) VALUES ({placeholders})"
    ...
    def _export(self):
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM qbox_data")
...
def run_qbox(urls=None, limit: int = 100) -> list[dict]:
    ...
    try:
        ...
        if os.path.exists(DB_PATH):
            ...
            conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM qbox_data LIMIT ?", (limit,))
            results = [dict(row) for row in cursor.fetchall()]
            conn.close()
    except Exception as e:
        spider_instance.logger.error(f"Error in run_qbox: {e}")
