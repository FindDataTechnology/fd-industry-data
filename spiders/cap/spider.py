...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "cap.db")
OUTPUT_DIR = os.path.join(os.path.dirname(__file__), "output")

START_URLS = [
    "https://www.cap.gov.cn/",
]
CUSTOM_UAS = [
...
class CapSpider(Spider):
    ...
    def _save_sqlite(self, item):
        ...
        try:
            ...
            create_sql = f"""CREATE TABLE IF NOT EXISTS cap_data (
            ...
            insert_sql = f"INSERT OR IGNORE INTO cap_data ({', '.join(cols)}) VALUES ({placeholders})"
    ...
    def _export(self):
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM cap_data")
