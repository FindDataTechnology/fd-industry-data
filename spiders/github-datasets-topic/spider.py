#!/usr/bin/env python3
"""GitHub Dataset Topics Scrapling Spider."""
...
def save_to_sqlite(records: List[Dict[str, Any]]) -> None:
    ...
    if not records:
        conn.close()
        return
    sql_create = """CREATE TABLE IF NOT EXISTS github_datasets_topic (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        scraped_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        source_url TEXT, repo_name TEXT, repo_url TEXT,
        description TEXT, language TEXT, stars TEXT,
        forks TEXT, topics TEXT
    )"""
    cursor.execute(sql_create)
    for record in records:
        cursor.execute(
            "INSERT INTO github_datasets_topic (source_url, repo_name, repo_url, description, language, stars, forks, topics) VALUES (?,?,?,?,?,?,?,?)",
            (record.get("source_url"), record.get("repo_name"), record.get("repo_url"),
             record.get("description"), record.get("language"), record.get("stars"),
             record.get("forks"), record.get("topics"))
        )
    conn.commit()
    conn.close()
...
def parse_response(response: Response) -> List[Dict[str, Any]]:
    ...
    try:
        repo_items = response.css(".repo-list-item, article.Box-row, [data-hydro-click]")
        for item in repo_items:
            link = item.css("h3 a, .repo-list-name a")
            desc = item.css(".repo-list-description::text, p::text")
            stars = item.css(".octicon-star + span::text, [aria-label*=star] + span::text")
            language = item.css("[itemprop=programmingLanguage]::text")
            parsed_data = {
                "source_url": response.url,
                "repo_name": link[0].css("::text").get("").strip() if link else "",
                "repo_url": "https://github.com" + link[0].attrib.get("href", "") if link else "",
                "description": desc.get("").strip() if desc else "",
                "language": language.get("").strip() if language else "",
                "stars": stars.get("").strip() if stars else "",
                "forks": "",
                "topics": ", ".join(item.css(".topic-tag::text").getall()),
        ...
        if not results:
            results.append({
                "source_url": response.url,
                "repo_name": response.css("title::text").get(""),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Scrapling spider for GitHub Dataset Topics")
