...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "gtarsc.db")
...
class GtarscSpider(Spider):
    ...
    def _save_sqlite(self, item):
        ...
        try:
            ...
            create_sql = f"""CREATE TABLE IF NOT EXISTS gtarsc_data (
            ...
            insert_sql = f"INSERT OR IGNORE INTO gtarsc_data ({', '.join(cols)}) VALUES ({placeholders})"
    ...
    def _export(self):
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM gtarsc_data")
...
def run_gtarsc(urls=None, save=True):
    """Run the gtarsc spider. Returns True on success."""
    spider = GtarscSpider()
