...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "wanfangdata.db")
...
class WanfangdataSpider(Spider):
    name = "wanfangdata"
    ...
    def _save_sqlite(self, item):
        ...
        try:
            ...
            create_sql = f"""CREATE TABLE IF NOT EXISTS wanfangdata_data (
            ...
            insert_sql = f"INSERT OR IGNORE INTO wanfangdata_data ({', '.join(cols)}) VALUES ({placeholders})"
    ...
    def _export(self):
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM wanfangdata_data")
...
def run_wanfangdata(urls=None, limit: int = 100) -> list[dict]:
    ...
    spider_instance = WanfangdataSpider()
    ...
    try:
        ...
        if os.path.exists(DB_PATH):
            ...
            conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM wanfangdata_data LIMIT ?", (limit,))
            results = [dict(row) for row in cursor.fetchall()]
            conn.close()
    except Exception as e:
        spider_instance.logger.error(f"Error in run_wanfangdata: {e}")
