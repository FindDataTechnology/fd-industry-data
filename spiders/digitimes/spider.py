...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "digitimes.db")
OUTPUT_DIR = os.path.join(os.path.dirname(__file__), "output")

START_URLS = [
    "https://www.digitimes.com/research/",
]
CUSTOM_UAS = [
...
class DigitimesSpider(Spider):
    ...
    def _save_sqlite(self, item):
        ...
        try:
            ...
            create_sql = f"""CREATE TABLE IF NOT EXISTS digitimes_data (
            ...
            insert_sql = f"INSERT OR IGNORE INTO digitimes_data ({', '.join(cols)}) VALUES ({placeholders})"
    ...
    def _export(self):
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM digitimes_data")
...
def run_digitimes(limit: int | None = None) -> list[dict]:
...
if __name__ == "__main__":
    ...
    spider_instance = DigitimesSpider()
