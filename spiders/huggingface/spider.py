...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "huggingface.db")
...
class HuggingfaceSpider(Spider):
    name = "huggingface"
    ...
    def _save_sqlite(self, item):
        ...
        try:
            ...
            create_sql = f"""CREATE TABLE IF NOT EXISTS huggingface_data (
            ...
            insert_sql = f"INSERT OR IGNORE INTO huggingface_data ({', '.join(cols)}) VALUES ({placeholders})"
    ...
    def _export(self):
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM huggingface_data")
...
if __name__ == "__main__":
    ...
    spider_instance = HuggingfaceSpider()
