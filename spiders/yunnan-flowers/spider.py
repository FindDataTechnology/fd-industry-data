...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "yunnan-flowers.db")
...
class YunnanflowersSpider(Spider):
    name = "yunnan-flowers"
    ...
    def _save_sqlite(self, item):
        ...
        try:
            ...
            create_sql = f"""CREATE TABLE IF NOT EXISTS yunnan-flowers_data (
            ...
            insert_sql = f"INSERT OR IGNORE INTO yunnan-flowers_data ({', '.join(cols)}) VALUES ({placeholders})"
    ...
    def _export(self):
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM yunnan-flowers_data")
...
def run_yunnan_flowers(urls=None, limit: int = 100) -> list[dict]:
    ...
    spider_instance = YunnanflowersSpider()
    ...
    try:
        ...
        if os.path.exists(DB_PATH):
            ...
            conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM `yunnan-flowers_data` LIMIT ?", (limit,))
            results = [dict(row) for row in cursor.fetchall()]
            conn.close()
    except Exception as e:
        spider_instance.logger.error(f"Error in run_yunnan_flowers: {e}")
