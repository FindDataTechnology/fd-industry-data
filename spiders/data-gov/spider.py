...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "data-gov.db")
...
class DatagovSpider(Spider):
    ...
    def _save_sqlite(self, item):
        ...
        try:
            ...
            create_sql = f"""CREATE TABLE IF NOT EXISTS data-gov_data (
            ...
            insert_sql = f"INSERT OR IGNORE INTO data-gov_data ({', '.join(cols)}) VALUES ({placeholders})"
    ...
    def _export(self):
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM data-gov_data")
...
def run_data_gov(limit: int | None = None) -> list[dict]:
