...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "yn-agriculture.db")
OUTPUT_DIR = os.path.join(os.path.dirname(__file__), "output")

START_URLS = [
    "http://nongye.yn.gov.cn",
]
CUSTOM_UAS = [
...
class YnagricultureSpider(Spider):
    name = "yn-agriculture"
    ...
    def _save_sqlite(self, item):
        ...
        try:
            ...
            create_sql = f"""CREATE TABLE IF NOT EXISTS yn-agriculture_data (
            ...
            insert_sql = f"INSERT OR IGNORE INTO yn-agriculture_data ({', '.join(cols)}) VALUES ({placeholders})"
    ...
    def _export(self):
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM yn-agriculture_data")
...
def run_yn_agriculture(urls=None, limit: int = 100) -> list[dict]:
    ...
    spider_instance = YnagricultureSpider()
    ...
    try:
        ...
        if os.path.exists(DB_PATH):
            ...
            conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM `yn-agriculture_data` LIMIT ?", (limit,))
            results = [dict(row) for row in cursor.fetchall()]
            conn.close()
    except Exception as e:
        spider_instance.logger.error(f"Error in run_yn_agriculture: {e}")
