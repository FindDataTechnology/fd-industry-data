...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "wechat-mp.db")
...
class WechatmpSpider(Spider):
    ...
    def _save_sqlite(self, item):
        ...
        try:
            ...
            create_sql = f"""CREATE TABLE IF NOT EXISTS wechat-mp_data (
            ...
            insert_sql = f"INSERT OR IGNORE INTO wechat-mp_data ({', '.join(cols)}) VALUES ({placeholders})"
    ...
    def _export(self):
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM wechat-mp_data")
...
def run_wechat_mp(urls=None, limit: int = 100) -> list[dict]:
    ...
    spider_instance = WechatmpSpider()
    ...
    try:
        ...
        if os.path.exists(DB_PATH):
            ...
            conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM `wechat-mp_data` LIMIT ?", (limit,))
            results = [dict(row) for row in cursor.fetchall()]
            conn.close()
    except Exception as e:
        spider_instance.logger.error(f"Error in run_wechat_mp: {e}")
