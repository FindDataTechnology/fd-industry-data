...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "github-akshare.db")
...
NAME = "AkShare Financial Library"

class GithubakshareSpider(Spider):
    name = "github-akshare"
    ...
    def _save_sqlite(self, item):
        ...
        try:
            ...
            create_sql = f"""CREATE TABLE IF NOT EXISTS akshare_info (
            ...
            insert_sql = f"INSERT OR IGNORE INTO akshare_info ({col_str}) VALUES ({placeholders})".format(
                col_str=", ".join(cols), 
    ...
    def _export(self):
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM akshare_info")
...
if __name__ == "__main__":
    ...
    spider_instance = GithubakshareSpider()
