...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "isee.db")
OUTPUT_DIR = os.path.join(os.path.dirname(__file__), "output")

START_URLS = [
    "https://www.isee.gov.cn/",
]
CUSTOM_UAS = [
...
class IseeSpider(Spider):
    ...
    def _save_sqlite(self, item):
        ...
        try:
            ...
            create_sql = f"""CREATE TABLE IF NOT EXISTS isee_data (
            ...
            insert_sql = f"INSERT OR IGNORE INTO isee_data ({', '.join(cols)}) VALUES ({placeholders})"
    ...
    def _export(self):
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM isee_data")
...
def run_isee(urls=None, save=True):
    """Run the isee spider. Returns True on success."""
    spider = IseeSpider()
