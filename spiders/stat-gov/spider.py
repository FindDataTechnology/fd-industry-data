...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "stat-gov.db")
OUTPUT_DIR = os.path.join(os.path.dirname(__file__), "output")

START_URLS = [
    "https://www.stat.gov.cn/",
]
CUSTOM_UAS = [
...
class StatgovSpider(Spider):
    ...
    def _save_sqlite(self, item):
        ...
        try:
            ...
            create_sql = f"""CREATE TABLE IF NOT EXISTS stat-gov_data (
            ...
            insert_sql = f"INSERT OR IGNORE INTO stat-gov_data ({', '.join(cols)}) VALUES ({placeholders})"
    ...
    def _export(self):
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM stat-gov_data")
...
def run_stat_gov(urls=None, limit: int = 100) -> list[dict]:
    ...
    try:
        ...
        if os.path.exists(DB_PATH):
            ...
            conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM `stat-gov_data` LIMIT ?", (limit,))
            results = [dict(row) for row in cursor.fetchall()]
            conn.close()
    except Exception as e:
        spider_instance.logger.error(f"Error in run_stat_gov: {e}")
