...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "cssn.db")
...
class CssnSpider(Spider):
    ...
    def _save_sqlite(self, item):
        ...
        try:
            ...
            create_sql = f"""CREATE TABLE IF NOT EXISTS cssn_data (
            ...
            insert_sql = f"INSERT OR IGNORE INTO cssn_data ({', '.join(cols)}) VALUES ({placeholders})"
    ...
    def _export(self):
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM cssn_data")
...
def run_cssn(limit: int | None = None) -> list[dict]:
