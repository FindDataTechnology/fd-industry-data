...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "cqvip.db")
...
class CqvipSpider(Spider):
    ...
    def _save_sqlite(self, item):
        ...
        try:
            ...
            create_sql = f"""CREATE TABLE IF NOT EXISTS cqvip_data (
            ...
            insert_sql = f"INSERT OR IGNORE INTO cqvip_data ({', '.join(cols)}) VALUES ({placeholders})"
    ...
    def _export(self):
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM cqvip_data")
...
def run_cqvip(limit: int | None = None) -> list[dict]:
