#!/usr/bin/env python3
"""GitHub China Topics Scrapling Spider."""
...
def save_to_sqlite(records: List[Dict[str, Any]]) -> None:
    ...
    for key, value in sample.items():
        ...
        placeholders.append("?")
    table_name = "github_china_topics_records"
...
def parse_response(response: Response) -> List[Dict[str, Any]]:
    ...
    try:
        repo_items = response.css("article.Box-row")
        if not repo_items:
            repo_items = response.css("[data-testid='results-list'] > div")
        if not repo_items:
            repo_items = response.css(".repo-list-item")

        for item in repo_items:
            record: Dict[str, Any] = {}

            name_el = item.css("h1 a, h2 a, h3 a")
            if name_el:
                record["repo_name"] = name_el[0].text.strip() if hasattr(name_el[0], "text") else str(name_el[0])
                href = name_el[0].attrib.get("href", "") if hasattr(name_el[0], "attrib") else ""
                record["repo_url"] = "https://github.com" + href if href else ""

            desc_el = item.css("p")
            if desc_el:
                record["description"] = desc_el[0].text.strip() if hasattr(desc_el[0], "text") else str(desc_el[0])

            stars_el = item.css("a[href$='/stargazers'], [aria-label*='star']")
            if stars_el:
                record["stars"] = stars_el[0].text.strip() if hasattr(stars_el[0], "text") else str(stars_el[0])

            lang_el = item.css("[itemprop='programmingLanguage']")
            if lang_el:
                record["language"] = lang_el[0].text.strip() if hasattr(lang_el[0], "text") else str(lang_el[0])

            record["source_url"] = response.request.url
        ...
        if not results:
            parsed_data = {
                ...
                "page_title": "GitHub China Topics",
        ...
        while retry_count < MAX_RETRIES:
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning("HTTP {} for {}, retry {}/{}".format(response.status_code, url, retry_count + 1, MAX_RETRIES))
            except Exception as e:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Scrapling spider for GitHub China Topics")
