...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "intrinio.db")
...
class IntrinioSpider(Spider):
    ...
    def _save_sqlite(self, item):
        ...
        try:
            ...
            create_sql = f"""CREATE TABLE IF NOT EXISTS intrinio_data (
            ...
            insert_sql = f"INSERT OR IGNORE INTO intrinio_data ({', '.join(cols)}) VALUES ({placeholders})"
    ...
    def _export(self):
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM intrinio_data")
...
def run_intrinio(urls=None, save=True):
    """Run the intrinio spider. Returns True on success."""
    spider = IntrinioSpider()
...
if __name__ == "__main__":
    ...
    spider_instance = IntrinioSpider()
