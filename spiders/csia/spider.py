...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "csia.db")
...
class CsiaSpider(Spider):
    ...
    def _save_sqlite(self, item):
        ...
        try:
            ...
            create_sql = f"""CREATE TABLE IF NOT EXISTS csia_data (
            ...
            insert_sql = f"INSERT OR IGNORE INTO csia_data ({', '.join(cols)}) VALUES ({placeholders})"
    ...
    def _export(self):
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM csia_data")
...
def run_csia(limit: int | None = None) -> list[dict]:
