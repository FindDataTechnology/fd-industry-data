...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "polygon.db")
...
class PolygonSpider(Spider):
    ...
    def _save_sqlite(self, item):
        ...
        try:
            ...
            create_sql = f"""CREATE TABLE IF NOT EXISTS polygon_api_info (
            ...
            insert_sql = f"INSERT OR IGNORE INTO polygon_api_info ({col_str}) VALUES ({placeholders})".format(
                col_str=", ".join(cols), 
    ...
    def _export(self):
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM polygon_api_info")
