...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "cisia.db")
OUTPUT_DIR = os.path.join(os.path.dirname(__file__), "output")

START_URLS = [
    "http://www.cisia.org.cn/",
]
CUSTOM_UAS = [
...
class CisiaSpider(Spider):
    ...
    def _save_sqlite(self, item):
        ...
        try:
            ...
            create_sql = f"""CREATE TABLE IF NOT EXISTS cisia_data (
            ...
            insert_sql = f"INSERT OR IGNORE INTO cisia_data ({', '.join(cols)}) VALUES ({placeholders})"
    ...
    def _export(self):
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM cisia_data")
