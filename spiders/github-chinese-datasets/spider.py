#!/usr/bin/env python3
"""GitHub Chinese Datasets Scrapling Spider."""
...
START_URLS: List[str] = [
    "https://github.com/datasets/chinese-datasets",
...
def save_to_sqlite(records: List[Dict[str, Any]]) -> None:
    ...
    if not records:
        conn.close()
        return
    sql_create = """CREATE TABLE IF NOT EXISTS github_chinese_datasets (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        scraped_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        source_url TEXT, dataset_name TEXT, description TEXT,
        dataset_url TEXT, category TEXT, format_type TEXT,
        size TEXT
    )"""
    cursor.execute(sql_create)
    for record in records:
        cursor.execute(
            "INSERT INTO github_chinese_datasets (source_url, dataset_name, description, dataset_url, category, format_type, size) VALUES (?,?,?,?,?,?,?)",
            (record.get("source_url"), record.get("dataset_name"), record.get("description"),
             record.get("dataset_url"), record.get("category"), record.get("format_type"),
             record.get("size"))
        )
    conn.commit()
    conn.close()
...
def parse_response(response: Response) -> List[Dict[str, Any]]:
    ...
    try:
                ...
                if tag in ("h2", "h3"):
                    ...
                    if link:
                        parsed_data = {
                            ...
                            "dataset_url": link[0].attrib.get("href", ""),
        ...
        if not results:
            repo_items = response.css(".repo-list-item, article.Box-row")
            for item in repo_items:
                link = item.css("h3 a")
                desc = item.css("p::text")
                parsed_data = {
                    "source_url": response.url,
                    "dataset_name": link[0].css("::text").get("").strip() if link else "",
                    "description": desc.get("").strip() if desc else "",
                    "dataset_url": "https://github.com" + link[0].attrib.get("href", "") if link else "",
                    "category": "",
                    "format_type": "",
                    "size": "",
                    "scraped_at": datetime.now().isoformat(),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Scrapling spider for GitHub Chinese Datasets")
