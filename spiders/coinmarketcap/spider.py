...
DB_PATH = os.path.join(os.path.dirname(__file__), "data", "coinmarketcap.db")
...
class CoinmarketcapSpider(Spider):
    ...
    def _save_sqlite(self, item):
        ...
        try:
            ...
            create_sql = f"""CREATE TABLE IF NOT EXISTS coinmarketcap_data (
            ...
            insert_sql = f"INSERT OR IGNORE INTO coinmarketcap_data ({', '.join(cols)}) VALUES ({placeholders})"
    ...
    def _export(self):
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM coinmarketcap_data")
